...
<head>
    ...
    <title>Clínica Salud | Listado de categorias</title>
</head>
<body>
    ...
    <section id="content">
        ...
        <main>
            ...
            <ul class="breadcrumbs">
                ...
                <li class="divider">></li>
                <li><a href="#" class="active">Listado de las categorias</a></li>
            </ul>
            <button class="button" onclick="location.href='categoria_nuevo.php'">Nuevo</button>
          <div class="data">
...
$sentencia = $connect->prepare("SELECT * FROM category ORDER BY idcat DESC;");
 $sentencia->execute();
$data =  array();
if($sentencia){
         ...
         <table id="example" class="responsive-table">
            ...
            <tbody>
                <?php foreach($data as $d):?>
                    <tr>
                        <th scope="row"><?php echo $d->nomcat ?></th>
                        
                        <td data-title="Estado">
    
                        <label class="switch">
                          <input type="checkbox" id="<?=$d->idcat?>" value="<?=$d->state ?>" <?=$d->state == '1' ? 'checked' : '' ;?>/> 

                          <span class="slider"></span>
                        </label>
                        </td>
                        <td>
                <a title="Actualizar" href="../medicinas/categoria_editar.php?id=<?php echo $d->idcat ?>" class="fa fa-pencil tooltip"></a>
                            <a title="Información" href="../medicinas/categoria_info.php?id=<?php echo $d->idcat ?>" class="fa fa-info"></a>
                           
                            
                            <form  onsubmit="return confirm('Realmente desea eliminar el registro?');" method='POST' action='<?php $_SERVER['PHP_SELF'] ?>'>
<input type='hidden' name='idcat' value="<?php echo  $d->idcat; ?>">
